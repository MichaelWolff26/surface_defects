...
def DataReader(
    data_path,
):  # Reading dataframe and image-data, return x_batch, y_batch as torch tensors
    df1 = create_label_list(data_path=data_path)
    img_size = torchvision.io.read_image(
        data_path + df1["file_name"][1]
    ).size()  # get image size
    x_batch_size = (
        2 * len(df1)
    )  # x_batch_size is 2 times the length of the original data, because of the flipping operation
    x_batch = torch.zeros(
        x_batch_size, img_size[0], img_size[1], img_size[2], dtype=torch.float32
    )  # creating x_batch
    ...
    for i in range(
        len(df1)
    ):  # read the images to x_batch, crop them to size, normalize the values (/255) and flip them; assign labels to y_batch
